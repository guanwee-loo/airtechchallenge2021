...
with open('star.json','r') as handle:
...
merge (a{{i.value}}) <- [:HAS_ROUTE_FROM {SID : "{{ rec["name"] }}"} ] - (wp{{0}}{{i.value}})
{% for wp_i in range(1,wp_ctr) %} 
merge (wp{{wp_i}}{{i.value}}: Waypoint { name : "{{ rec["waypoints"][wp_i].name }}", uid : "{{ rec["waypoints"][wp_i].name }}" })
merge (wp{{wp_i - 1}}{{i.value}}) <- [:HAS_ROUTE_FROM {SID : "{{ rec["name"] }}"} ] - (wp{{wp_i}}{{i.value}})
